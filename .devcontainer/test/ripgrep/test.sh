...
# Test 5: db2-dark theme colors are in config (default theme)
check "db2-dark theme colors are configured" grep -q "#4A90C8" "$HOME/.config/ripgrep/config"
...
check "theme files are installed" test -f "/usr/local/share/devcontainer-features/ripgrep/themes/db2-dark.theme"
